...
from src.application.use_cases.identity import permissions_for_roles
from src.domain.enums import PrincipalType
from src.domain.errors import UnauthorizedError
from src.domain.models import AuthContext, DeploymentRunner, Principal, ReleaseSource
...
def auth_context_for_principal(principal: Principal, claims: dict[str, object] | None = None) -> AuthContext:
    return AuthContext(
    ...
    *,
    ...
    for runner in runners:
        if runner.token_hash == token_hash:
            ...
            return auth_context_for_principal(principal, {"runnerId": runner.runner_id})
    for release_source in release_sources:
        if release_source.token_hash == token_hash:
            ...
            if principal is None or not principal.active or principal.type != PrincipalType.SERVICE or principal.auth_method != "pat":
                raise UnauthorizedError("PAT principal is inactive or not registered.")
            return auth_context_for_principal(principal, {"releaseSourceId": release_source.release_source_id})
